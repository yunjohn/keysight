...
class ScopeMainWindow(QMainWindow):
    ...
    def _build_ui(self) -> None:
        ...
        acquire_layout = QGridLayout(acquire_box)
        acquire_layout.setContentsMargins(12, 10, 12, 10)
        acquire_layout.setHorizontalSpacing(8)
        acquire_layout.setVerticalSpacing(6)
        ...
        acquire_layout.addWidget(QLabel("采集类型"), 0, 0)
        acquire_layout.addWidget(self.acquire_type_combo, 0, 1)
        acquire_layout.addWidget(self.run_button, 1, 0)
        acquire_layout.addWidget(self.stop_button, 1, 1)
        acquire_layout.setColumnStretch(2, 1)
        ...
        top_row.addWidget(QLabel("测量通道"), 0, 0)
        top_row.addWidget(self.channel_combo, 0, 1)
        top_row.addWidget(QLabel("轮询间隔(s)"), 0, 2)
        top_row.addWidget(self.interval_input, 0, 3)
        top_row.addWidget(self.single_button, 0, 4)
        top_row.addWidget(self.auto_measure_button, 0, 5)
        top_row.addWidget(self.measurement_status, 1, 0, 1, 3)
        top_row.addWidget(self.last_update_value, 1, 3, 1, 3)
        top_row.setColumnStretch(6, 1)
        measure_layout.addLayout(top_row)

        selection_row = QGridLayout()
        selection_row.setHorizontalSpacing(8)
        selection_row.setVerticalSpacing(6)
        ...
        selection_row.addWidget(QLabel("测量项"), 0, 0)
        selection_row.addWidget(self.select_default_button, 0, 1)
        selection_row.addWidget(self.select_all_button, 0, 2)
        selection_row.addWidget(self.clear_selection_button, 0, 3)
        selection_row.addWidget(self.measurement_count_label, 1, 0, 1, 4)
        selection_row.setColumnStretch(4, 1)
        ...
        template_row.setVerticalSpacing(6)
        template_row.addWidget(QLabel("测量模板"), 0, 0)
        for template_index, template_name in enumerate(MEASUREMENT_TEMPLATES):
            ...
            template_row.addWidget(button, 0, template_index + 1)
        template_row.setColumnStretch(len(MEASUREMENT_TEMPLATES) + 1, 1)
        measure_layout.addLayout(template_row)

        waveform_row = QGridLayout()
        waveform_row.setHorizontalSpacing(8)
        waveform_row.setVerticalSpacing(6)
        ...
        waveform_row.addWidget(QLabel("波形模式"), 0, 0)
        waveform_row.addWidget(self.waveform_mode_combo, 0, 1)
        waveform_row.addWidget(QLabel("点数"), 0, 2)
        waveform_row.addWidget(self.waveform_points_input, 0, 3)
        waveform_row.addWidget(self.fetch_waveform_button, 1, 0)
        waveform_row.addWidget(self.load_waveform_button, 1, 1)
        waveform_row.addWidget(self.recent_waveform_button, 1, 2)
        waveform_row.addWidget(self.export_waveform_button, 1, 3)
        waveform_row.setColumnStretch(4, 1)
        ...
        scope_display_row = QGridLayout()
        scope_display_row.setHorizontalSpacing(8)
        scope_display_row.setVerticalSpacing(6)
        scope_display_row.addWidget(QLabel("示波器通道"), 0, 0)
        for channel_index, channel in enumerate(SUPPORTED_CHANNELS):
            ...
            scope_display_row.addWidget(checkbox, 0, channel_index + 1)
        scope_display_row.setColumnStretch(len(SUPPORTED_CHANNELS) + 1, 1)
        measure_layout.addLayout(scope_display_row)

        unit_row = QGridLayout()
        unit_row.setHorizontalSpacing(8)
        unit_row.setVerticalSpacing(6)
        unit_row.addWidget(QLabel("通道单位"), 0, 0)
        for channel_index, channel in enumerate(SUPPORTED_CHANNELS):
            unit_row.addWidget(QLabel(display_channel_name(channel)), 0, channel_index + 1)
            combo = QComboBox()
            ...
            unit_row.addWidget(combo, 1, channel_index + 1)
        unit_row.setColumnStretch(len(SUPPORTED_CHANNELS) + 1, 1)
        ...
        screenshot_action_row = QGridLayout()
        screenshot_action_row.setHorizontalSpacing(8)
        screenshot_action_row.setVerticalSpacing(6)
        ...
        screenshot_action_row.addWidget(QLabel("截图前缀"), 0, 0)
        screenshot_action_row.addWidget(self.screenshot_prefix_input, 0, 1)
        screenshot_action_row.addWidget(self.capture_button, 1, 0, 1, 2)
        screenshot_action_row.setColumnStretch(1, 1)
